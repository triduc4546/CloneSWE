...
export default function FeaturedProducts({ onSelectProduct }: any) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      where("category", "in", ["Cà phê chai Success", "Cà phê hòa tan"]),
      limit(12)
    )
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10">
          {products.map((p) => (
            <div
              ...
              className="bg-white border border-zinc-300 hover:border-yellow-500 transition-all
